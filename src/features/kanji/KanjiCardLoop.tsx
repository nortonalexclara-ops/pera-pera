...
interface KanjiCardLoopProps {
  ...
  // 'review' : uniquement les kanjis marqués "À revoir" (voir
  // ReviewMarkRecord, db.ts).
...
export default function KanjiCardLoop({
  ...
  const reviewIds = useLiveQuery(
    () => (profileId ? getReviewIds(profileId, 'kanji') : Promise.resolve(EMPTY_SET)),
    [profileId],
    EMPTY_SET,
  )
  ...
  // En 'mix', kanjiList ne dépend pas de masteredIds/reviewIds donc c'est
  // déjà bon dès le premier rendu. En 'new'/'review', il faut attendre que
  // useLiveQuery ait résolu sa vraie valeur (pas EMPTY_SET, sa valeur par
  // défaut le temps du chargement) — sinon CardLoopShell figerait la file
  // sur "tout le niveau" au lieu du sous-ensemble attendu.
  const itemsReady =
    ...
    <CardLoopShell
      ...
      emptyDescription={
        allMastered
          ...
          : noneToReview
            ? `Aucun kanji${level ? ` ${level}` : ''} marqué "À revoir" pour l'instant.`
